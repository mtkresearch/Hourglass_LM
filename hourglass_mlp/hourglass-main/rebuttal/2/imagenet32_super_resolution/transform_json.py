...
def transform_json(input_data):
    ...
    for param_count_key, top_value in input_data.items():
        ...
        for model_type, model_data in top_value.items():
            ...
            if new_model_type not in output_data["bs512_ep2_aug4"]:
                output_data["bs512_ep2_aug4"][new_model_type] = {}
            ...
            for config_key, config_value in model_data.items():
                ...
                if new_config_key not in output_data["bs512_ep2_aug4"][new_model_type]:
                    output_data["bs512_ep2_aug4"][new_model_type][new_config_key] = {}
                
                for lr_key, lr_value in config_value.items():
                    if lr_key not in output_data["bs512_ep2_aug4"][new_model_type][new_config_key]:
                        output_data["bs512_ep2_aug4"][new_model_type][new_config_key][lr_key] = {}
                    ...
                    for run_key, run_value in lr_value.items():
                        ...
                        if "metrics" in run_value and "psnr_batch" in run_value["metrics"]:
                            ...
                            output_data["bs512_ep2_aug4"][new_model_type][new_config_key][lr_key][run_key] = {
                                "metrics": {
                                    "test_psnr": [psnr_batch_value] if psnr_batch_value is not None else []
                                }
                            }
