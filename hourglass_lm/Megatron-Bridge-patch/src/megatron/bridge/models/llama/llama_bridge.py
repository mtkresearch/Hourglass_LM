...
# from transformers import LlamaForCausalLM # need to be changed
from megatron.bridge.models.llama.hf_transformers.modeling_llama import LlamaForCausalLM
...
class LlamaBridge(MegatronModelBridge):
    ...
    def provider_bridge(self, hf_pretrained: PreTrainedCausalLM) -> GPTModelProvider:
        ...
        # new
        # custom fields
        provider.num_mlp_layers = getattr(hf_config, "num_mlp_layers", 1)
        provider.mlp_intermediate_sizes = getattr(hf_config, "mlp_intermediate_sizes", [11008])
        provider.mlp_use_residual_proj = getattr(hf_config, "mlp_use_residual_proj", True)
    ...
    def megatron_to_hf_config(cls, provider: GPTModelProvider) -> dict:
        ...
        # new
        # Add custom fields to HF config
        hf_config["num_mlp_layers"] = provider.num_mlp_layers
        hf_config["mlp_intermediate_sizes"] = provider.mlp_intermediate_sizes
        hf_config["mlp_use_residual_proj"] = provider.mlp_use_residual_proj
    ...
    def mapping_registry(self) -> MegatronMappingRegistry:
        ...
        param_mappings = {
            ...
            "decoder.layers.*.mlp.*.linear_fc1.layer_norm_weight": "model.layers.*.post_attention_layernorm.*.weight",  # te implementation # modified
            ...
            "decoder.layers.*.mlp.*.linear_fc2.weight": "model.layers.*.mlp.*.down_proj.weight", # modified
        }
