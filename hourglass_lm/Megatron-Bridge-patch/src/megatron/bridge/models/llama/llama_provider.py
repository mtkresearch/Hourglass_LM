...
# ============================================================================
# Custom MLP Layer Specs for extending Llama models with multi-layer MLPs
# ============================================================================

def get_custom_mlp_layer_spec(config: "Llama32ModelProvider1B") -> ModuleSpec:
    """Create a layer spec that uses custom multi-layer MLP.
    
    This spec is based on the standard Llama spec but replaces the standard
    single-layer MLP with a custom MultiLayerMLP that allows:
    - Multiple sequential MLP layers
    - Residual connections between layers
    - Configurable intermediate sizes
    
    Args:
        config: The Llama model provider configuration
        
    Returns:
        ModuleSpec: Layer specification with custom MLP
    """
    from megatron.bridge.models.llama.llama_custom_mlp import MultiLayerMLP
    
    # Get the base Llama layer spec
    ...
    # Replace the MLP module with our custom implementation
    if hasattr(base_spec, 'submodules') and hasattr(base_spec.submodules, 'mlp'):
        # Override the MLP submodule spec
        # NOTE: Do NOT pass 'config' in params — build_module() in TransformerLayer
        # already passes config=self.config as a kwarg. Passing it here too would
        # cause "got multiple values for argument 'config'" TypeError.
        base_spec.submodules.mlp = ModuleSpec(
            module=MultiLayerMLP,
        )
...
@dataclass
class Llama32ModelProvider1BWithCustomMLP(Llama32ModelProvider1B):
    """Llama 3.2 1B with custom multi-layer MLP architecture.
    
    This extends the standard 1B provider to use a custom MLP with:
    - Multiple internal MLP layers (default: 3)
    - Residual connections between MLP layers
    - Customizable intermediate dimensions for each layer
    
    Configuration example:
        provider = Llama32ModelProvider1BWithCustomMLP()
        provider.num_mlp_layers = 5  # Use 5 internal MLP layers
        provider.mlp_intermediate_sizes = [8192, 6144, 4096, 6144, 8192]  # Optional
        provider.mlp_use_residual_proj = True  # Enable residuals
    """
    
    # Number of sequential MLP layers within the FFN block
    num_mlp_layers: int = 3
    
    # Optional: specify different intermediate sizes for each MLP layer
    # If not specified, will use config.ffn_hidden_size for all layers
    mlp_intermediate_sizes: Optional[List[int]] = None
    
    # Whether to use residual connections between MLP layers
    mlp_use_residual_proj: bool = True
    
    # Use the custom MLP layer spec
    transformer_layer_spec: Union[ModuleSpec, Callable] = field(
        default_factory=lambda: get_custom_mlp_layer_spec
    )
    
    def __post_init__(self):
        """Post-init to handle the layer spec callable."""
        # If transformer_layer_spec is a callable, keep it as is
        # The provide() method in GPTModelProvider will call it with self as argument
        pass
