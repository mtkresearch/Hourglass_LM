...
def _print_model_structure(model):
    """Print model structure with focus on MLP layers (called once at training start).
    
    This helps verify that custom MLP is being used instead of standard FFN.
    """
    # print("\n" + "="*100)
    # print("[MODEL STRUCTURE] Printing model parameters and MLP configuration")
    # print("="*100)
    
    # # Track MLP-related parameters
    # mlp_param_names = []
    # mlp_layer_count = 0
    # multimlp_count = 0
    
    # print("\nAll module names (searching for MLP-related modules):\n")
    
    # for name, module in model.named_modules():
    #     # Print all modules with "mlp" in the name
    #     if 'mlp' in name.lower():
    #         module_type = type(module).__name__
    #         num_params = sum(p.numel() for p in module.parameters())
            
    #         # Highlight custom MultiLayerMLP
    #         if 'MultiLayerMLP' in module_type:
    #             multimlp_count += 1
    #             print(f"✓ {name}")
    #             print(f"  Type: {module_type} [CUSTOM MLP]")
    #             print(f"  Parameters: {num_params:,}")
    #             if hasattr(module, 'num_mlp_layers'):
    #                 print(f"  Num sub-layers: {module.num_mlp_layers}")
    #             if hasattr(module, 'mlp_intermediate_sizes'):
    #                 print(f"  Intermediate sizes: {module.mlp_intermediate_sizes}")
    #             if hasattr(module, 'use_residual_proj'):
    #                 print(f"  Use residual connections: {module.use_residual_proj}")
    #         else:
    #             mlp_layer_count += 1
    #             print(f"  {name}: {module_type} ({num_params:,} params)")
    ...
    # Total model parameter counts
    total_params = sum(p.numel() for p in model.parameters())
    total_mlp_params = sum(p.numel() for n, p in model.named_parameters() if 'mlp' in n.lower())
    total_attn_params = sum(p.numel() for n, p in model.named_parameters()
                           if 'self_attention' in n.lower() or 'attention' in n.lower() or 'input_layernorm' in n.lower())
    # Avoid double-counting: remove any params that are both 'attention' and 'mlp'
    attn_and_mlp_overlap = sum(p.numel() for n, p in model.named_parameters()
                               if ('self_attention' in n.lower() or 'attention' in n.lower())
                               and 'mlp' in n.lower())
    total_attn_params -= attn_and_mlp_overlap
    non_mlp_params = total_params - total_mlp_params
    other_params = non_mlp_params - total_attn_params  # Embedding + LayerNorm + output_layer etc.
    ...
    print("SUMMARY:")
    print(f"\n  --- Parameter Counts ---")
    print(f"  Total model parameters:  {total_params:>14,} ({_fmt_params(total_params)})")
    print(f"  MLP parameters:          {total_mlp_params:>14,} ({_fmt_params(total_mlp_params)}, {total_mlp_params/(total_mlp_params+total_attn_params)*100:.1f}% of MLP+ATTN)")
    print(f"  Attention parameters:    {total_attn_params:>14,} ({_fmt_params(total_attn_params)}, {total_attn_params/(total_mlp_params+total_attn_params)*100:.1f}% of MLP+ATTN)")
    print(f"  MLP + ATTN:              {total_mlp_params+total_attn_params:>14,} ({_fmt_params(total_mlp_params+total_attn_params)})")
    print(f"  Other parameters:        {other_params:>14,} ({_fmt_params(other_params)}, Embedding + LayerNorm + output)")
    
    
    print("="*100 + "\n")


def _print_attn_mlp_params(model):
    """Per-parameter breakdown of the first Transformer layer's attention and MLP blocks.

    Prints every parameter (name, shape, numel) grouped by:
      - self_attention.*  → ATTN block
      - mlp.*            → MLP block

    This is meant to be diffed against calculate_detailed_model_size.py's output
    to pinpoint which module causes a discrepancy.
    """
    # Find the first decoder layer
    first_layer = None
    for name, module in model.named_modules():
        # Megatron: layers are at decoder.layers.0
        if name.endswith('.layers.0') or name == 'decoder.layers.0':
            first_layer = (name, module)
            break
    if first_layer is None:
        print("[ATTN/MLP DEBUG] Could not find decoder.layers.0")
        return

    layer_name, layer = first_layer
    attn_params = {}
    mlp_params = {}

    for pname, param in layer.named_parameters():
        if pname.startswith('self_attention.') or pname.startswith('input_layernorm.'):
            attn_params[pname] = param
        elif pname.startswith('mlp.'):
            mlp_params[pname] = param

    W = 80
    print("\n" + "=" * W)
    print(f"[ATTN/MLP DEBUG] Layer: {layer_name}  (one layer, before ×n_layers)")
    print("=" * W)

    print("\n--- ATTENTION ---")
    attn_total = 0
    for pname, p in sorted(attn_params.items()):
        n = p.numel()
        attn_total += n
        print(f"  {pname:<55} {str(list(p.shape)):<25} {n:>10,d}")
    print(f"  {'ATTN TOTAL':<55} {'':25} {attn_total:>10,d}")

    print("\n--- MLP ---")
    mlp_total = 0
    for pname, p in sorted(mlp_params.items()):
        n = p.numel()
        mlp_total += n
        print(f"  {pname:<55} {str(list(p.shape)):<25} {n:>10,d}")
    print(f"  {'MLP TOTAL':<55} {'':25} {mlp_total:>10,d}")

    print(f"\n  ATTN + MLP (per layer): {attn_total + mlp_total:,d}")
    print("=" * W + "\n")
...
def forward_step(
    ...
    """
    # Print model architecture on first call
    if not hasattr(forward_step, '_model_printed'):
        _print_model_structure(model)
        _print_attn_mlp_params(model)
        forward_step._model_printed = True
