...
class GPTDataset(MegatronDataset):
    ...
    def __init__(
        ...
        # DEBUG: print the first 10 samples after index construction
        _num_debug = min(10, len(self))
        log_single_rank(logger, logging.INFO, f"[DEBUG] GPTDataset first {_num_debug} samples ({self.index_split.name}):")
        for _i in range(_num_debug):
            _sample = self[_i]
            _tokens = _sample["tokens"].tolist()
            log_single_rank(logger, logging.INFO, f"  [{_i}] tokens[:20]={_tokens[:20]}")
